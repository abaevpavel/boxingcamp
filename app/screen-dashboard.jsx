    ...
    <window.Screen title={`${block.name} · Week ${wk}`} sub={`${plan.deload ? 'Deload week · ' : ''}${block.phase} · ${window.TOTAL_CAMP_WEEKS}-wk camp`}
      ...
      <window.Card style={{ marginBottom: 14 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 18 }}>
          <div>
            <div style={{ fontSize: 64, fontWeight: 800, letterSpacing: -3, lineHeight: 0.95 }}>{daysOut}</div>
            <window.Label style={{ marginTop: 5 }}>days to fight</window.Label>
          </div>
          <div style={{ textAlign: 'right', whiteSpace: 'nowrap' }}>
            <window.Label style={{ textAlign: 'right' }}>Today</window.Label>
            <div style={{ fontSize: 15, color: T.text, fontWeight: 800, marginTop: 3 }}>{window.fmtWeekday(today)} · {window.fmtShort(today)}</div>
            <window.Label style={{ textAlign: 'right', marginTop: 12 }}>Fight day</window.Label>
            <div style={{ fontSize: 15, color: T.text, fontWeight: 800, marginTop: 3 }}>{window.fmtWeekday(window.FIGHT_DATE)} · {window.fmtShort(window.FIGHT_DATE)}</div>
          </div>
        </div>
        <div style={{ height: 1, background: T.line, margin: '0 0 16px' }} />
          ...
          {[
            ...
            <React.Fragment key={c.label}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 38, fontWeight: 800, lineHeight: 1, letterSpacing: -1.2, marginTop: 4 }}>{c.val}<span style={{ fontSize: 16, color: T.muted, marginLeft: 3 }}>{c.unit}</span></div>

...
function WeightSheet({ open, onClose }) {
  ...
  const bfTgt = window.targetBodyFat(d);
  const bfDelta = bf ? +(bfTgt - +bf).toFixed(1) : null;
  ...
  return (
    <window.Sheet open={open} onClose={onClose} title="Log weight">
      <window.Field label="Weight (lb)">
        <input type="number" inputMode="decimal" autoFocus value={w} onChange={e => setW(e.target.value)} placeholder="214.6" style={{ ...window.inputStyle, fontSize: 26, fontWeight: 800, padding: '18px 16px', letterSpacing: -0.5 }} />
        {delta != null && (
          <div style={{ marginTop: 10, fontSize: 14.5, fontWeight: 700, color: delta >= 0 ? T.accent : T.warn }}>
            Target {tgt.toFixed(1)} · {Math.abs(delta) < 0.1 ? 'dead on the curve' : `${Math.abs(delta).toFixed(1)} lb ${delta >= 0 ? 'ahead' : 'behind'}`}
          </div>
        )}
      </window.Field>
      <window.Field label="Body fat % (optional)">
        <input type="number" inputMode="decimal" value={bf} onChange={e => setBf(e.target.value)} placeholder="manual or scanner" style={{ ...window.inputStyle, fontSize: 26, fontWeight: 800, padding: '18px 16px', letterSpacing: -0.5 }} />
        {bfDelta != null && (
          <div style={{ marginTop: 10, fontSize: 14.5, fontWeight: 700, color: bfDelta >= 0 ? T.accent : T.warn }}>
            Target {bfTgt.toFixed(1)}% · {Math.abs(bfDelta) < 0.1 ? 'on pace' : `${Math.abs(bfDelta).toFixed(1)} pts ${bfDelta >= 0 ? 'ahead' : 'behind'}`}
          </div>
        )}
      </window.Field>
      <window.Field label="Date"><input type="date" value={d} min="2026-05-18" max="2026-09-16" onChange={e => setD(e.target.value)} style={{ ...dateInputStyle(), fontSize: 20, fontWeight: 700, padding: '18px 16px' }} /></window.Field>

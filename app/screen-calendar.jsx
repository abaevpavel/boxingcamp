    ...
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
        {Array.from({ length: 7 }).map((_, i) => {
          ...
          const block = window.blockForDate(iso);
          const rows = window.daySessions(iso, state).slice().sort((a, b) => {
          ...
          return (
            <div key={iso} onClick={() => onDay(iso)} style={{
              background: T.card, border: `1px solid ${isToday ? T.accent + '66' : T.line}`, borderRadius: 14,
              padding: '9px 14px', cursor: 'pointer', borderLeft: `4px solid ${isFight ? T.accent : block ? block.color : T.line2}`,
            }}>
              ...
              {rows.length === 0 ? (
                <div style={{ fontSize: 12.5, color: T.faint, fontWeight: 600, marginTop: 2 }}>Rest</div>
              ) : (

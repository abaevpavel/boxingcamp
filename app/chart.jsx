...
function WeightChart({ weights, from, to, height = 220, showBodyFat = true, showAxis = true, padTop = 14 }) {
  ...
  const onMove = e => {
    ...
    let f = ((cx - rect.left) / rect.width * W - padL) / (W - padL - padR);
    f = Math.max(0, Math.min(1, f));
    setScrub(window.addDays(from, Math.round(f * total)));
  };
  const handlers = showAxis ? { onPointerMove: onMove, onPointerDown: onMove, onPointerLeave: () => setScrub(null), onPointerUp: () => setScrub(null) } : {};
  const sx = scrub ? xFor(scrub) : null;
  const scTw = scrub ? window.targetWeight(scrub) : null;
  const scBf = scrub ? window.targetBodyFat(scrub) : null;
  const scActual = scrub ? inRange.find(w => w.date === scrub) : null;

  return (
    <div ref={wrapRef} style={{ position: 'relative', touchAction: 'none', cursor: showAxis ? 'crosshair' : 'default' }} {...handlers}>
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={H} style={{ display: 'block', overflow: 'visible' }}>
      ...
      {/* (today marker intentionally hidden) */}
      {/* target curve */}
      ...
      {/* scrub markers */}
      {sx != null && <line x1={sx} y1={padT} x2={sx} y2={H - padB} stroke={T.text} strokeWidth="1" opacity="0.45" />}
      {sx != null && <circle cx={sx} cy={yFor(scTw)} r="4.5" fill={T.bg} stroke={T.accent} strokeWidth="2.4" />}
      {sx != null && showBodyFat && <circle cx={sx} cy={yBf(scBf)} r="4.5" fill={T.bg} stroke={bfColor} strokeWidth="2.4" />}
    </svg>
    {scrub && showAxis && (
      <div style={{ position: 'absolute', top: 0, left: `${Math.max(9, Math.min(91, sx / W * 100))}%`, transform: 'translateX(-50%)', pointerEvents: 'none', background: T.card2, border: `1px solid ${T.line2}`, borderRadius: 10, padding: '7px 11px', boxShadow: '0 6px 18px rgba(0,0,0,0.35)', whiteSpace: 'nowrap', textAlign: 'center', zIndex: 5 }}>
        <div style={{ fontFamily: T.font, fontSize: 11.5, fontWeight: 800, color: T.text }}>{window.fmtWeekday(scrub)} {window.fmtShort(scrub)}</div>
        <div style={{ fontFamily: T.mono, fontSize: 11, fontWeight: 700, color: T.accent, marginTop: 3 }}>target {scTw.toFixed(1)} lb</div>
        {showBodyFat && <div style={{ fontFamily: T.mono, fontSize: 11, fontWeight: 700, color: bfColor, marginTop: 1 }}>target {scBf.toFixed(1)}%</div>}
        {scActual && <div style={{ fontFamily: T.mono, fontSize: 10, fontWeight: 600, color: T.muted, marginTop: 3 }}>logged {scActual.weightLb}{scActual.bodyFatPct != null ? ` · ${scActual.bodyFatPct}%` : ''}</div>}
      </div>
    )}
    </div>
  );

    ...
    <window.Screen title="Weight" sub="Accountability"
      ...
      <window.Card style={{ marginBottom: 14 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          <div>
            ...
            <div style={{ fontSize: 56, fontWeight: 800, lineHeight: 1, letterSpacing: -2, marginTop: 4 }}>{wd ? wd.weight.toFixed(1) : '—'}<span style={{ fontSize: 20, color: T.muted, marginLeft: 6 }}>lb</span></div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <window.Label style={{ textAlign: 'right' }}>Vs curve</window.Label>
            <div style={{ fontSize: 34, fontWeight: 800, lineHeight: 1, marginTop: 6, color: wd && wd.ahead ? T.accent : T.warn }}>{wd ? (wd.delta >= 0 ? '−' : '+') + Math.abs(wd.delta).toFixed(1) : '—'}</div>
            <div style={{ fontSize: 12.5, fontWeight: 700, color: wd && wd.ahead ? T.accent : T.warn, marginTop: 2 }}>{wd && wd.ahead ? 'AHEAD' : 'BEHIND'}</div>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 0, marginTop: 16, borderTop: `1px solid ${T.line}`, paddingTop: 14 }}>
      ...
      <window.Card style={{ marginBottom: 14 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          <div>
            ...
            <div style={{ fontSize: 44, fontWeight: 800, lineHeight: 1, letterSpacing: -1.5, marginTop: 4, color: latestBf ? T.text : T.faint }}>
              {latestBf ? latestBf.bodyFatPct.toFixed(1) : '—'}<span style={{ fontSize: 18, color: T.muted, marginLeft: 4 }}>%</span>
            </div>
            <div style={{ fontSize: 12.5, color: T.muted, fontWeight: 600, marginTop: 4 }}>
              {latestBf ? `last measured ${window.fmtShort(latestBf.date)} · optional` : 'optional — manual or scanner'}
            </div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <window.Label style={{ textAlign: 'right' }}>Since start</window.Label>
            <div style={{ fontSize: 30, fontWeight: 800, lineHeight: 1, marginTop: 6, color: bfChange != null ? (bfChange <= 0 ? T.accent : T.warn) : T.faint }}>
              {bfChange != null ? (bfChange <= 0 ? '−' : '+') + Math.abs(bfChange).toFixed(1) : '—'}
            </div>
            <window.Label style={{ textAlign: 'right', marginTop: 2 }}>points</window.Label>
          </div>
        </div>
        {bfEntries.length < 2 && (
          <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px solid ${T.line}`, fontSize: 12.5, color: T.faint, fontWeight: 600 }}>
            Add a body-fat reading when you weigh in to start tracking the trend.
          </div>
        )}
      </window.Card>
      ...
      <window.Card style={{ marginBottom: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 16, marginTop: 10, justifyContent: 'center' }}>
          ...
          {showBf && <Legend color="#7b86c2" label="Body fat %" />}
        </div>
      </window.Card>
